...
import com.lucky.game.vo.client.GameEntityVo;
import com.lucky.game.vo.client.MarketDetailVo;
import com.lucky.game.vo.client.MarketInfoVo;
...
public class GameBiz {
    ...
    public void startDigitalCashGame() {
        GameEntity gameEntity = gameService.findByStartTime(DateUtils.getCurrDateMmss(), DictEnum.GAME_STATUS_01.getCode());
        if (gameEntity != null) {
    ...
    public void endDigitalCashGame() {
        Date currentDate = DateUtils.getCurrDateMmss();
        ...
        if (gameEntity != null) {
            ...
            Date lastEndTime = DateUtils.addMinute(gameEntity.getEndTime(), -1);
    ...
    public void initDigitalCashGame() {
        ...
        Date tomorrow = DateUtils.addDay(DateUtils.getCurrDate(), 0);
        Date initDate = DateUtils.getTodayZero(tomorrow);
        Date startTime;
        Date endTime;
        String code;
        int count;
        GameEntity lastGameEntity = gameService.findLastRecordByType(DictEnum.GAME_TYPE_02.getCode());
        if (lastGameEntity != null && lastGameEntity.getCreateTime().after(DateUtils.getCurrDate())) {
            log.info("today game is already create!,lastGameEntity createTime={}", lastGameEntity.getCreateTime());
        } else {
            for (int i = 0; i < time; i++) {
                count = i + 1;
                startTime = DateUtils.addMinute(initDate, i * timeFrequency);
                endTime = DateUtils.addMinute(startTime, playFrequency);
                code = DateUtils.getCurrentDate() + count;
